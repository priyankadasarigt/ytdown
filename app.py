...
def get_ydl_base_opts():
    ...
    # Tell yt-dlp to use Node.js as the JS runtime (installed in Dockerfile)
    node_path = '/usr/bin/node'
    if os.path.exists(node_path):
        opts['js_runtimes'] = [f'node:{node_path}']
        print(f"[yt-dlp] Using Node.js runtime: {node_path}")
    else:
        print("[yt-dlp] Node.js not found at /usr/bin/node")
